...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-2 z-50 w-screen px-4">
      <div
        className="conainer flex items-center justify-between rounded-lg
        bg-black py-3"
      >
        <div className="flex flex-shrink-0 items-center ml-5 justify-between">
          <Image
            className="mr-2"
            src="/logo.png"
            width={60}
        ...
        <div className="hidden lg:flex">
          <ul className="flex items-center gap-4">
            {NAV_LINKS.map((item, index) => (
              <li key={index}>
                <Link
                  className="text-sm text-white hover:text-neutral-500 pr-3"
                  href={item.url}
                >
                  {item.title}
                </Link>
        ...
        <div className="hidden text-sm text-white lg:flex">
          ...
          <Button className="bg-slate-800 mr-2">Sign up</Button>
        </div>
        <div className="flex-col justify-end mr-2 text-white md:flex lg:hidden">
